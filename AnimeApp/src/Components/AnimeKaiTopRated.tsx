    ...
    return (
      <div className="p-4 text-center">
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
          <p className="text-red-400 text-sm">AnimeKai Top Rated temporarily unavailable</p>
          ...
          <WatchOverlay key={anime.id} id={anime.id} finder={anime.title} name={anime.title}>
            <div className="group bg-[#16162a] border border-[#2d2d4a] rounded-lg overflow-hidden hover:border-purple-600 hover:-translate-y-1 transition-all duration-200 cursor-pointer">
              <div className="relative aspect-[2/3] overflow-hidden">
                ...
                {anime.rank && anime.rank <= 3 && (
                  <div className={`absolute top-1.5 right-1.5 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white
                    ${anime.rank === 1 ? 'bg-yellow-500' : anime.rank === 2 ? 'bg-gray-400' : 'bg-orange-600'}`}>
                    #{anime.rank}
                  </div>
                )}
